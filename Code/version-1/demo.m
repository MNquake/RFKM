...
data = load('G:\Data\dataset\Yale32_nosiy.mat','X');
label = load('G:\Data\dataset\Yale32_nosiy.mat','Y');
...
while 0<1
    ...
    center = zeros(row,k);
    [F, obj_RFKM, iter, center] = newRFKM(F, r, data, center, noise, K,S);
%     close all;
    [~,max_F] = max(F,[],2);
    [idx,~] = find(S(:,1)~=0);
    max_F = max_F(idx,:);
